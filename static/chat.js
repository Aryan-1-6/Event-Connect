...
if (author == myName){
    addMessageFromSelf(author, msg);
} else {
    addMessageFromOtherUser(author, msg);
}
var height =  $( '#messages-div' ).height() + 10;
$( '#messages-div' ).scrollTop(height);
})

socket.on('someone connected', function(Name, ) {
...
// addUserToOnlineDiv(usersOnlineDisplayNames[i], usersOnlineAvatars[i])
        ...
        addUserToOnlineDiv(Name)
        addUserChangeMessage(Name, true)
    } // end else
